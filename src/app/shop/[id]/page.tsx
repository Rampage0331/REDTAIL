    ...
    <div className="pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          ...
          {/* Image placeholder */}
          <div className="aspect-[3/4] bg-[#1c1410] flex items-center justify-center">
          ...
          <div className="flex flex-col justify-center">
            {product.collection && (
              <p className="text-xs tracking-[0.3em] text-[#8b1212] mb-4">{product.collection.toUpperCase()}</p>
            )}
            ...
            <div className="mb-10">
              ...
              <div className="flex flex-wrap gap-2">
                {product.sizes.map((s) => (
                  <button key={s} className="w-14 h-14 border border-stone-700 text-xs tracking-widest text-stone-400 hover:border-[#8b1212] hover:text-stone-100 transition-colors">
                    {s}
                  </button>
                ))}
              </div>
                ...
                <ul className="space-y-2">
                  {product.details.map((d) => (
                    <li key={d} className="text-stone-400 text-sm flex items-start gap-3">
                      <span className="text-[#8b1212] mt-1">—</span>{d}
                    </li>
                  ))}
                </ul>
              </>
